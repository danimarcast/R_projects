...
muestrax<-rbind(comp_1,comp_2,comp_3)
datos<-as.data.frame(muestrax)
names(datos)<-c("x1","x2","Componente")

color<-c("#E495A5","#7DB0DD","#D099FF")
...
mezcla<-Mclust(datos[indices,-3],G=3)
...
plot(densidad,what = "density",col="#36D9EA",lwd=2,add=TRUE)
legend(-2,12,legend = c(TeX("\\textbf{COMP 1}"),TeX("\\textbf{COMP 2}"),TeX("\\textbf{COMP 3}"),TeX("\\textbf{GMM}")),lty = 1,col = c("blue","red","#2D29A5","#36D9EA"),lwd=4,pch=3,cex=0.5,bty="n")

plot(densidad,what = "density",type="persp",col="#36D9EA")
title(TeX("Distribución GMM con \\textit{$K=3$}"))
